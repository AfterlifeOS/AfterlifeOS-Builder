...
if [ ! -d "$KEYS_DIR" ]; then
    ...
    echo "Please ensure keys are synced via manifest to vendor/afterlife-priv/keys"
    exit 1
fi
...
AFTERLIFE_BUILD_NAME="AfterlifeOS_${DEVICE}-${VERSION_LOWER}"
...
# --- PROSES EXTRACTION ---
echo "Extracting target files to avoid Python ZipFile bugs..."
TEMP_DIR="${OUT}/temp_target_files"
rm -rf "$TEMP_DIR"
mkdir -p "$TEMP_DIR"
unzip -q "$TARGET_FILES_ZIP" -d "$TEMP_DIR"
...
sign_target_files_apks -o -d ${KEYS_DIR} \
    ...
    "$TEMP_DIR" \
    "${OUT}/${AFTERLIFE_BUILD_NAME}-signed-target_files.zip" || { echo "Signing Failed!"; rm -rf "$TEMP_DIR"; exit 1; }

# Bersihkan temp folder SETELAH signing selesai
rm -rf "$TEMP_DIR"
...
ota_from_target_files -k ${KEYS_DIR}/releasekey \
    --block --backup=true \
    "${OUT}/${AFTERLIFE_BUILD_NAME}-signed-target_files.zip" \
    "${OUT}/${AFTERLIFE_BUILD_NAME}.zip" || { echo "Packaging Failed!"; exit 1; }
